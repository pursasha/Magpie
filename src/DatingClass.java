...
public class DatingClass 
{
	...
	public static String getResponse1(String statement) //gets customized responses from user when a certain keyword is said/
	{
		...
		String response = "";
		if (Tools.findKeyword(statement, "Date",0) >= 0) //uses the method findkeyword in class miscellaneous.
		{
			response = "Do you want to go on a computer date? Yes or no?";
			x=1;
		}
		else if (Tools.findKeyword(statement, "yes",0) >=0 && (x==1))// This x==1 will make sure that this doesnt run when they arent on a date.
		{
			response = "Of course you do. Everyone does. Do you think I am hot ?";	
			x=2;
		}
		else if(Tools.findKeyword(statement, "no",0) >=0 && (x==1)) // Numbers allow for the robot to have a conversation 
		{
			response = "I dont take no for an answer. Do you think I am hot? "; 
			x=2;
		}
		else if(Tools.findKeyword(statement, "yes",0) >=0 && (x==2))
		{
			response = "Well obviousely I am hot, I am sizzling. What about you? Are you sizzling hot? "; 
			x=3;
		}
		
		else if(Tools.findKeyword(statement, "no",0) >=0 && (x==2))
		{
			response = "I think you mistook me for yourself. I am sizzling hot. What about you? Are you sizzling hot? ";
			x=3;
		}
		else if(Tools.findKeyword(statement, "yes",0) >=0 && (x==3))
		{
			response = "Dont even play yourself. Try taking a look in the mirror. ";
			x=4;
		}
		else if(Tools.findKeyword(statement, "no",0) >=0 && (x==3))
		{
